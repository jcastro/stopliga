...
| `UNIFI_USERNAME` | Sí | - | Usuario para login local |
| `UNIFI_PASSWORD` | Sí | - | Contraseña para login local |
...
StopLiga ya usa la API local del router. Lo que no se puede evitar es autenticarse.
...
Hoy no estoy usando API key local para estas routes porque no tengo confirmación oficial de que los endpoints concretos de Traffic Routes que estamos usando acepten ese método en todas las versiones. Si lo validamos en tu controlador exacto, se puede añadir como opción.
...
- si no hay `STOPLIGA_VPN_NAME` ni `STOPLIGA_TARGETS`, usa la primera VPN cliente disponible y el primer cliente disponible de forma determinista
